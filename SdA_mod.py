...
def train_SdA(train, minimum, n_features, hidden_sizes,
             corruption_levels, pretraining_epochs=15, pretrain_lr=0.001, batch_size=10):
    """
    ...
    hidden_sizes.append(n_features)
    if len(hidden_sizes) > len(corruption_levels):
        print ('not enough corruption levels specified, must be the same number of the hidden layers, exiting now...')
        exit()
    sda = SdA(
        numpy_rng=numpy_rng,
        n_ins=minimum*6,

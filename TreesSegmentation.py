...
Output:
	- the functions will store the csv corresponding to each tree in the out_path folder in which both the oiginal RGB values
       and a specific color assigned to the points for visualization purposes are available

'''
def treesSegmentation(area_index, out_path, distance_threshold):
    	"""Function to segment individual trees in a given 'las' forested area file"""
    
    # printing starting messages
    	print 'starting clustering area'
    	print 'the distance threshold is ' + str(distance_threshold)
    
    # path where to store clusters
    	path = out_path + "Trees Area %i" % (area)
